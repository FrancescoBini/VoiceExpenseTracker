...
export default function TransactionsCurtain({ isOpen, onClose, transactions, selectedMonth }: TransactionsCurtainProps) {
  ...
  const sortedTransactions = [...localTransactions].sort((a, b) => b.timestamp - a.timestamp);
  ...
  const handleDelete = async (transaction: Transaction) => {
    ...
    try {
      const result = await deleteTransaction(transaction, selectedMonth);
      if (result) {
        // Remove the transaction from local state immediately
        setLocalTransactions(prev => prev.filter(t => t.id !== transaction.id));
        console.log('Transaction removed from UI:', transaction.id);
      } else {
  ...
  return (
    <div
      className={`fixed top-0 left-0 w-[634px] h-full bg-gray-900 transform transition-transform duration-300 ease-in-out z-50 ${
